	...
	return {
		exec: async (command, cwd, { onData, signal, timeout, env }) => {
			...
			try {
				...
				// A shell killed by a signal has no exit code. Map it to the shell convention of
				// 128 + signal number (137 for SIGKILL) so callers do not mistake it for success.
				const signalCode = child.signalCode;
				return { exitCode: exitCode ?? (signalCode ? 128 + (osConstants.signals[signalCode] ?? 0) : null) };
			} finally {
